...
// This is a simple turtle controller node that uses ROS 2 C++ API
// to control a turtle in the turtlesim simulator.

class TurtleController : public rclcpp::Node
{
public:
    TurtleController()
        : Node("turtle_controller")   // 1.调用父类构造函数，设置节点名称
    {
        // 2.创建发布者和订阅者
        // 发布速度消息到 /turtle1/cmd_vel topic
        // 订阅海龟的位置信息 /turtle1/pose
        velocity_publisher_ = this->create_publisher<geometry_msgs::msg::Twist>("/turtle1/cmd_vel", 10);
        pose_subscription_  = this->create_subscription<turtlesim::msg::Pose>(
            "/turtle1/pose", 10, std::bind(&TurtleController::on_pose_received_, this, std::placeholders::_1));
        // 3.创建服务
        // param1: 服务名称
        // param2: 服务回调函数，接收请求和响应
        // param3: 服务选项，默认为空
        // param4: 服务回调组，默认为空
        ...
        parameters_callback_handle_ = this->add_on_set_parameters_callback(
            [&](const std::vector<rclcpp::Parameter>& params) -> SetParametersResult {
                ...
                {
                    RCLCPP_INFO(
                        this->get_logger(), "更新参数 %s 值为：%f", param.get_name().c_str(), param.as_double());
                    if (param.get_name() == "k")
                    {
                        k_ = param.as_double();
                    }
    ...
    void patrol_callback(const std::shared_ptr<Patrol::Request> request, std::shared_ptr<Patrol::Response> response)
    {
        // 判断巡逻点是否在模拟器边界内
